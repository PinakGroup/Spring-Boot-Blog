...
- 改用 `<textarea>` 来存放 Markdown 源文档；
    ...
    - 创建 Editor.md 只需要写一个 `<div articleId="xxxx"></div>` ，如果没有添加 `class="editormd"` 属性会自动添加，另外如果不存在 `<textarea>` 标签，则也会自动添加 `<textarea>` ；
...
- 修复 Bug [#23](https://github.com/pandao/editor.md/issues/23)，即 Headers 的 articleId 属性的重复及中文问题；
